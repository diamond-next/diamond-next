...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "src", "collectors"))
)
...
def get_collector_tests(path, verbose=True):
    for f in os.listdir(path):
        ...
        if os.path.isfile(fn) and f.startswith("test") and f.endswith(".py"):
            ...
            try:
                # Import the module
                collectorTests[modname] = __import__(
                    modname, globals(), locals(), ["*"]
                )
            except Exception:
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-v", "--verbose", dest="verbose", default=1, action="count", help="verbose"
    )
    ...
    cPath = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "src", "collectors", options.collector)
    )
