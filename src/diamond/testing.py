...
class CollectorTestCase(unittest.TestCase):
    ...
    def getFixtureDirPath(self):
        path = os.path.join(os.path.dirname(inspect.getfile(self.__class__)), 'fixtures')

        return path

    def getFixturePath(self, fixture_name):
        path = os.path.join(self.getFixtureDirPath(), fixture_name)
    ...
    def assertUnpublishedMany(self, mock, items, expected_value=0):
        return self.assertPublishedMany(mock, items, expected_value)

    def assertPublishedMany(self, mock, items, expected_value=1):
    ...
    def assertPublishedMetric(self, mock, key, value, expected_value=1):
        calls = list(filter(lambda x: x[0][0].path.find(key) != -1, mock.call_args_list))
    ...
    def assertUnpublishedMetricMany(self, mock, items, expected_value=0):
        return self.assertPublishedMetricMany(mock, items, expected_value)

    def assertPublishedMetricMany(self, mock, items, expected_value=1):
        for key, value in iter(items.items()):
