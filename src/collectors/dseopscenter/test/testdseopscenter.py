...
class TestDseOpsCenterCollector(CollectorTestCase):
    def setUp(self):
        config = get_collector_config(
            "DseOpsCenterCollector", {"cluster_id": "MyTestCluster"}
        )
    ...
    def test_should_work_with_real_data(self, publish_mock):
        urlopen_mock1 = patch(
            ...
            Mock(side_effect=lambda *args: self.getFixture("keyspaces.json")),
        )
        urlopen_mock1.start()
        ...
        urlopen_mock2 = patch(
            "urllib.request.urlopen",
            Mock(side_effect=lambda *args: self.getFixture("new-metrics.json")),
        )
        urlopen_mock2.start()
