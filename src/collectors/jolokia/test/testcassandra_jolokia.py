...
class TestCassandraJolokiaCollector(CollectorTestCase):
    ...
    def expected_fixture_a_p(self, percentile_key):
        return {"p25": 192.0, "p50": 398.0, "p75": 824.0, "p95": 2050.0, "p99": 2952.0}[
            percentile_key
        ]
    ...
    def test_should_compute_percentiles_accurately_when_empty(self):
        ninety_offsets = self.collector.create_offsets(90)
        self.assertEqual(
            self.collector.compute_percentile(
                ninety_offsets, self.empty_fixture_values(), 50
            ),
            0.0,
        )
        self.assertEqual(
            self.collector.compute_percentile(
                ninety_offsets, self.empty_fixture_values(), 95
            ),
            0.0,
        )
        self.assertEqual(
            self.collector.compute_percentile(
                ninety_offsets, self.empty_fixture_values(), 99
            ),
            0.0,
        )
    ...
    def test_should_collect_metrics_histogram_attributes(self, publish_mock):
        ...
        self.assertPublishedMany(
            publish_mock,
            {
                "RecentReadLatencyHistogramMicros.p50": self.expected_fixture_a_p(
                    "p50"
                ),
                "RecentReadLatencyHistogramMicros.p95": self.expected_fixture_a_p(
                    "p95"
                ),
                "RecentReadLatencyHistogramMicros.p99": self.expected_fixture_a_p(
                    "p99"
                ),
            },
        )
    ...
    def test_should_respect_percentiles_config(self, publish_mock):
        ...
        self.assertPublishedMany(
            publish_mock,
            {
                "RecentReadLatencyHistogramMicros.p25": self.expected_fixture_a_p(
                    "p25"
                ),
                "RecentReadLatencyHistogramMicros.p75": self.expected_fixture_a_p(
                    "p75"
                ),
            },
        )
    ...
    def test_should_respect_histogram_regex_config(self, publish_mock):
        ...
        self.assertPublishedMany(
            publish_mock,
            {
                "WackyMetricSeventeen.p50": self.expected_fixture_a_p("p50"),
                "WackyMetricSeventeen.p95": self.expected_fixture_a_p("p95"),
                "WackyMetricSeventeen.p99": self.expected_fixture_a_p("p99"),
            },
        )
