...
class TestProcessResourcesCollector(CollectorTestCase):
    TEST_CONFIG = {
        ...
        "process": {
            ...
            "barexe": {"exe": "bar$"},
            "noprocess": {"name": "noproc", "count_workers": "true"},
            "diamond-selfmon": {
                "selfmon": "true",
            },
        },
    ...
    def test(self, publish_mock, getpid_mock, time_mock):
        process_info_list = [
            ...
            {"exe": "/usr/bin/foo", "name": "bar", "pid": 9998, "rss": 1, "vms": 1},
            {"exe": "", "name": "barein", "pid": 9997, "rss": 2, "vms": 2},
            {
                "exe": "/usr/bin/bar",
                "name": "",
                "pid": 9996,
        ...
        process_iter_mock = (
            ProcessMock(
                pid=x["pid"], name=x["name"], rss=x["rss"], vms=x["vms"], exe=x["exe"]
            )
        ...
        patch_psutil_process_iter = patch(
            "psutil.process_iter", return_value=process_iter_mock
        )
