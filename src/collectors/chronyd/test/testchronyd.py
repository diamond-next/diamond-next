...
class TestChronydCollector(CollectorTestCase):
    ...
    def test_should_work_with_ip_addresses(self, publish_mock):
        patch_collector = patch.object(
            ChronydCollector,
            "get_output",
            Mock(return_value=self.getFixture("fedora").getvalue()),
        )
    ...
    def test_should_work_with_fqdns(self, publish_mock):
        patch_collector = patch.object(
            ChronydCollector,
            "get_output",
            Mock(return_value=self.getFixture("fqdn").getvalue()),
        )
        patch_collector.start()
        self.collector.collect()
        patch_collector.stop()
    ...
    def test_check_invalid_unit(self, publish_mock):
        patch_collector = patch.object(
            ChronydCollector,
            "get_output",
            Mock(return_value=self.getFixture("bad_unit").getvalue()),
        )
        patch_collector.start()
        self.collector.collect()
        patch_collector.stop()
    ...
    def test_huge_values(self, publish_mock):
        patch_collector = patch.object(
            ChronydCollector,
            "get_output",
            Mock(return_value=self.getFixture("huge_vals").getvalue()),
        )
        patch_collector.start()
        self.collector.collect()
        patch_collector.stop()
