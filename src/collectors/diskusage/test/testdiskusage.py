...
class TestDiskUsageCollector(CollectorTestCase):
    def setUp(self):
        config = get_collector_config(
            "DiskUsageCollector",
            {"interval": 10, "sector_size": "512", "byte_unit": "kilobyte"},
        )
    ...
    def test_get_disk_statistics(self):
        patch_open = patch(
            "builtins.open", Mock(return_value=self.getFixture("diskstats"))
        )
    ...
    def test_should_work_with_real_data(self, publish_mock):
        patch_open = patch(
            "builtins.open", Mock(return_value=self.getFixture("proc_diskstats_1"))
        )
        ...
        patch_open = patch(
            "builtins.open", Mock(return_value=self.getFixture("proc_diskstats_2"))
        )
    ...
    def test_verify_supporting_disk(self, publish_mock):
        patch_open = patch(
            "builtins.open", Mock(return_value=self.getFixture("proc_diskstats_1_disk"))
        )
        patch_time = patch("time.time", Mock(return_value=10))
        ...
        patch_open = patch(
            "builtins.open", Mock(return_value=self.getFixture("proc_diskstats_2_disk"))
        )
        patch_time = patch("time.time", Mock(return_value=20))
    ...
    def test_service_Time(self, publish_mock):
        patch_open = patch(
            "builtins.open",
            Mock(return_value=self.getFixture("proc_diskstats_1_service_time")),
        )
        patch_time = patch("time.time", Mock(return_value=10))
        ...
        patch_open = patch(
            "builtins.open",
            Mock(return_value=self.getFixture("proc_diskstats_2_service_time")),
        )
