...
class TestHttpdCollector(CollectorTestCase):
    ...
    def test_should_work_with_synthetic_data(self, publish_mock):
        ...
        patch_headers = patch.object(
            TestHTTPResponse, "getheaders", Mock(return_value={})
        )
        ...
        patch_read = patch.object(
            TestHTTPResponse,
            "read",
            Mock(return_value=self.getFixture("server-status-fake-2").getvalue()),
        )
    ...
    def test_should_work_with_real_data(self, publish_mock):
        ...
        patch_read = patch.object(
            TestHTTPResponse,
            "read",
            Mock(return_value=self.getFixture("server-status-live-1").getvalue()),
        )
        ...
        patch_read = patch.object(
            TestHTTPResponse,
            "read",
            Mock(return_value=self.getFixture("server-status-live-2").getvalue()),
        )
    ...
    def test_issue_456(self, publish_mock):
        ...
        patch_read = patch.object(
            TestHTTPResponse,
            "read",
            Mock(return_value=self.getFixture("server-status-live-3").getvalue()),
        )
        ...
        patch_read = patch.object(
            TestHTTPResponse,
            "read",
            Mock(return_value=self.getFixture("server-status-live-4").getvalue()),
        )
    ...
    def test_should_respect_max_redirect_limit_and_skip_url(self, publish_mock):
        self.setUp(
            config={
                "urls": [
                    "nickname1 http://localhost:9090/server-status?auto",
                    "nickname2 http://localhost:8080/server-status?auto",
                ],
            }
        )
        ...
        mock_getheaders.side_effect = [
            {"location": "http://localhost:8888/1"},
            {"location": "http://localhost:8888/2"},
            {"location": "http://localhost:8888/3"},
            {"location": "http://localhost:8888/4"},
            {"location": "http://localhost:8888/5"},
            {"location": "http://localhost:8888/6"},
            {},
        ] * 2

        patch_headers = patch.object(TestHTTPResponse, "getheaders", mock_getheaders)
