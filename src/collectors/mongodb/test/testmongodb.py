...
class TestMongoDBCollector(CollectorTestCase):
    def setUp(self):
        config = get_collector_config(
            "MongoDBCollector", {"host": "localhost:27017", "databases": "^db"}
        )
    ...
    @patch("pymongo.MongoClient")
    @patch.object(Collector, "publish")
    def test_should_publish_nested_keys_for_db_stats(
        self, publish_mock, connector_mock
    ):
    ...
    def test_should_ignore_unneeded_collections(self, publish_mock, connector_mock):
        ...
        assert (
            call("collstats", "tmp.mr.tmp1")
            not in self.connection["db1"].command.call_args_list
        )  # NOQA
        metrics = {
    ...
    @patch("pymongo.MongoClient")
    @patch.object(Collector, "publish")
    def test_should_ignore_replset_status_if_disabled(
        self, publish_mock, connector_mock
    ):
        data = {"more_keys": int(1), "key": 2, "string": "str"}
        self._annotate_connection(connector_mock, data)
        ...
        self.collector.collect()
        assert (
            call("replsetSetGetStatus")
            not in self.connection.admin.command.method_calls
        )
    ...
    @patch("pymongo.MongoClient")
    @patch.object(Collector, "publish")
    def test_should_publish_keys_from_real_server_stats(
        self, publish_mock, connector_mock
    ):
        ...
        self.assertEqual(
            len(dupes),
            0,
            "BUG: 1+ point for same metric received: %s" % ", ".join(dupes),
        )
...
class TestMongoDBCollectorWithReplica(CollectorTestCase):
    def setUp(self):
        config = get_collector_config(
            "MongoDBCollector",
            {"host": "localhost:27017", "databases": "^db", "replica": True},
        )
        self.collector = MongoDBCollector(config, None)
        self.connection = MagicMock()
    ...
    @patch("pymongo.MongoClient")
    @patch.object(Collector, "publish")
    def test_should_publish_replset_status_if_enabled(
        self, publish_mock, connector_mock
    ):
        data = {"more_keys": int(1), "key": 2, "string": "str"}
        self._annotate_connection(connector_mock, data)
