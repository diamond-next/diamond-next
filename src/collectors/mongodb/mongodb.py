...
class MongoDBCollector(diamond.collector.Collector):
        ...
        for host in hosts:
            ...
            try:
                ...
                if type(self.config["ssl"]) is str:
                    self.config["ssl"] = diamond.collector.str_to_bool(
                        self.config["ssl"]
                    )
            ...
            if user:
                try:
                    ...
                    self.log.error(
                        "User auth given, but could not autheticate with host: %s, err: %s"
                        % (host, e)
                    )
            ...
            for db_name in conn.database_names():
                ...
                for collection_name in conn[db_name].collection_names():
                    ...
                    if (
                        self.config["collection_sample_rate"] < 1
                    ...
                    if diamond.collector.str_to_bool(
                        self.config["translate_collections"]
                    ):
    ...
    def _publish_replset(self, data, base_prefix):
        ...
        healthy_nodes = functools.reduce(
            lambda value, node: value + node["health"], data["members"], 0
        )

        self._publish_dict_with_prefix(
            {"healthy_nodes": healthy_nodes, "total_nodes": total_nodes}, prefix
        )
    ...
    def _publish_transformed(self, data, base_prefix):
        ...
        self._publish_metrics(
            base_prefix + ["backgroundFlushing_per_sec"],
            "flushes",
        ...
        publish_percent(
            "indexCounters.btree.misses", "indexCounters.btree.accesses", data
        )
