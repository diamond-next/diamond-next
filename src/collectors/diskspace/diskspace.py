    ...
    def collect(self):
        ...
        for info in iter(results.values()):
            ...
            for unit in self.config['byte_unit']:
                ...
                metric_value = diamond.convertor.binary.convert(value=metric_value, old_unit='byte', new_unit=unit)
                self.publish_gauge(metric_name, metric_value, 2)
            ...
            if os.name != 'nt':
                if float(inodes_total) > 0:
                    self.publish_gauge('%s.inodes_percentfree' % name, float(inodes_free) / float(inodes_total) * 100)
