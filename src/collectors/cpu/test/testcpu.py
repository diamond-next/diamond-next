...
class TestCPUCollector(CollectorTestCase):
    def setUp(self):
        config = get_collector_config(
            "CPUCollector", {"interval": 10, "normalize": False}
        )
    ...
    def test_should_work_with_synthetic_data(self, publish_mock):
        patch_open = patch(
            "builtins.open",
            Mock(return_value=io.StringIO("cpu 100 200 300 400 500 0 0 0 0 0")),
        )
        ...
        patch_open = patch(
            "builtins.open",
            Mock(return_value=io.StringIO("cpu 110 220 330 440 550 0 0 0 0 0")),
        )
    ...
    def test_473(self, publish_mock):
        ...
        for call in publish_mock.mock_calls:
            ...
            if call[1] > 100:
                raise ValueError(
                    "metric %s: %s should not be over 100!" % (call[0], call[1])
                )
        ...
        for t in totals:
            ...
            if totals[t] >= 101:
                raise ValueError(
                    "metric total for %s: %s should not be over 100!" % (t, totals[t])
                )
        ...
        patch_open = patch(
            ...
            Mock(
                return_value=io.StringIO(
                    "\n".join(
                        [
                            self.input_dict_to_proc_string("", self.input_base),
        ...
        patch_open = patch(
            ...
            Mock(
                return_value=io.StringIO(
                    "\n".join(
                        [
                            self.input_dict_to_proc_string("", self.input_next),
