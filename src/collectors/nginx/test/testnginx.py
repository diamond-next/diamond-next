...
class TestNginxCollector(CollectorTestCase):
    ...
    def test_should_work_with_real_data(
        self, publish_counter_mock, publish_gauge_mock, publish_mock
    ):
        ...
        patch_urlopen = patch(
            "urllib.request.urlopen", Mock(return_value=mock_response)
        )
        ...
        self.assertPublishedMany(
            [publish_mock, publish_gauge_mock, publish_counter_mock], metrics
        )

    @patch.object(Collector, "publish")
    @patch.object(Collector, "publish_gauge")
    @patch.object(Collector, "publish_counter")
    def test_plus_should_work_with_real_data(
        self, publish_counter_mock, publish_gauge_mock, publish_mock
    ):
