...
class ElasticSearchCollector(diamond.collector.Collector):
    ...
    def _get(self, scheme, host, port, path, assert_key=None):
        ...
        if assert_key and assert_key not in doc:
            self.log.error("Bad response from elasticsearch, expected key '%s' was missing for %s" % (assert_key, url))
    ...
    def _index_metrics(self, metrics, prefix, index):
        ...
        self._add_metric(metrics, '%s.docs.deleted' % prefix, index, ['docs', 'deleted'])
        self._add_metric(metrics, '%s.datastore.size' % prefix, index, ['store', 'size_in_bytes'])

        # publish all 'total' and 'time_in_millis' stats
        self._copy_two_level(
            metrics, prefix, index,
            lambda key: key.endswith('total') or key.endswith('time_in_millis') or key.endswith('in_bytes')
        )  # noqa
    ...
    def collect_instance_cluster_stats(self, scheme, host, port, metrics):
        ...
        self._add_metric(metrics, 'cluster_health.nodes.pending_tasks', result, ['number_of_pending_tasks'])
        ...
        self._add_metric(metrics, 'cluster_health.shards.active_percent', result, ['active_shards_percent_as_number'])
        self._add_metric(metrics, 'cluster_health.shards.delayed_unassigned', result, ['delayed_unassigned_shards'])
    ...
    def collect_instance(self, alias, scheme, host, port):
        ...
        if 'cache' in indices:
            ...
            self._add_metric(metrics, 'cache.bloom.size', cache, ['bloom_size_in_bytes'])
            self._add_metric(metrics, 'cache.field.evictions', cache, ['field_evictions'])
            self._add_metric(metrics, 'cache.field.size', cache, ['field_size_in_bytes'])
            ...
            self._add_metric(metrics, 'cache.id.size', cache, ['id_cache_size_in_bytes'])
        ...
        if 'fielddata' in indices:
            ...
            self._add_metric(metrics, 'fielddata.size', fielddata, ['memory_size_in_bytes'])
        ...
        if 'segments' in indices:
            ...
            self._add_metric(metrics, 'segments.mem.size', segments, ['memory_in_bytes'])
            self._add_metric(metrics, 'segments.index_writer.mem.size', segments, ['index_writer_memory_in_bytes'])
            self._add_metric(metrics, 'segments.index_writer.mem.max_size', segments, ['index_writer_max_memory_in_bytes'])
            self._add_metric(metrics, 'segments.version_map.mem.size', segments, ['version_map_memory_in_bytes'])
            self._add_metric(metrics, 'segments.fixed_bit_set.mem.size', segments, ['fixed_bit_set_memory_in_bytes'])

        # process mem/cpu (may not be present, depending on access restrictions)
        self._add_metric(metrics, 'process.cpu.percent', data, ['process', 'cpu', 'percent'])
        self._add_metric(metrics, 'process.mem.resident', data, ['process', 'mem', 'resident_in_bytes'])
        self._add_metric(metrics, 'process.mem.share', data, ['process', 'mem', 'share_in_bytes'])
        self._add_metric(metrics, 'process.mem.virtual', data, ['process', 'mem', 'total_virtual_in_bytes'])
        ...
        if 'fs' in data and 'data' in data['fs'] and data['fs']['data']:
            ...
            self._add_metric(metrics, 'disk.reads.size', fs_data, ['disk_read_size_in_bytes'])
            self._add_metric(metrics, 'disk.writes.count', fs_data, ['disk_writes'])
            self._add_metric(metrics, 'disk.writes.size', fs_data, ['disk_write_size_in_bytes'])
